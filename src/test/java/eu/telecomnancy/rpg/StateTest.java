...
public class StateTest {
    ...
    public void testWoundedStateBehavior() {
        ...
        // Conan attaque Thulsa Doom (damage 2)
        int oldHP = doom.getHealth();
        conan.attack(doom);
        assertEquals(oldHP - 2, doom.getHealth(),
            "Doom doit perdre 2 HP si Conan attaque en WoundedState.");

        // Conan repasse en NormalState si on remonte ses HP >= 50
        conan.setHealth(55);
    ...
    public void testAttackCausesStateChangeInTarget() {
        ...
        doom.setHealth(12); // Pour illustrer une situation proche de la mort
        conan.attack(doom);
        // Doom doit perdre 10 HP => 2 HP restants
        assertEquals(7, doom.getHealth(),
            "Doom doit avoir 2 HP restants après l'attaque de Conan.");

        // On appelle update pour voir si doom change d'état
        // (il devrait passer en ScaredState car HP=2 < 10 et HP > 0)
        doom.update();
        assertTrue(doom.getState() instanceof ScaredState, "Doom doit passer en ScaredState avec 7 HP.");
    }
    ...
    public void testAutoChangeState() {
        ...
        conan.attack(doom); // => conan : NormalState -> WoundedState : donc doit infligé seulement 2 de dégâts
        assertEquals(28, doom.getHealth(), "Doom doit perdre 2 HP si Conan attaque en WoundedState.");
    ...
    public void testStateAndStrategy() {
        ...
        // Conan attaque Doom en mode Normal (5 de dégâts)
        // Mais on place Conan en DefensiveStrategy ( dégâts / 2 )
        ...
        assertEquals(28, doom.getHealth(),
            "Doom doit perdre 2 HP si Conan attaque en NormalState avec DefensiveStrategy.");

        // On place maintenant Conan en aggressiveStrategy (dégâts * 2)
        AggressiveStrategy attStrat = new AggressiveStrategy();
        conan.setStrategy(attStrat);
        conan.attack(doom);

        assertEquals(18, doom.getHealth(),
            "Doom doit perdre 10 HP si Conan attaque en NormalState avec AggressiveStrategy.");
    }
}
